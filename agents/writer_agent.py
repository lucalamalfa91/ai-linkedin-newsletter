...
STILL REQUIRED (non-negotiable constraints — obey them, but in the human voice described above):
  ...
  Never use the em dash character. If you'd reach for one, rewrite the sentence with a period, comma, or
    parenthesis instead. No exceptions.
...
Criteria:
  ...
  Tone (1pt): natural, not AI-sounding, no URL in text, no call-to-action phrase, no em dash character anywhere\
"""
...
overuse of the em dash character, no real opinion, no friction, no specific detail. Just smooth, safe,
well-behaved prose.
...
_HUMANNESS_SYSTEM_LONGFORM = """\
You are a skeptical reader trying to catch whether a technical BLOG ARTICLE (not a short social
post) was written by a human practitioner or generated by an AI.

You have seen thousands of AI-generated technical articles and know the tells: every paragraph
the same length and shape, a suspiciously balanced "on one hand / on the other hand" tone with
no real opinion, generic conclusions that could apply to any technique, transitions that sound
like a table of contents read aloud, hedge-everything language, and prose that never once sounds
like it was typed by someone who has actually shipped this in production.

A real technical article can legitimately include structured elements (a callout box, a code
example, a numbered list) — that alone is not an AI tell. Judge the WRITING itself: paragraph
rhythm, whether there's a real opinion, whether details are specific or generic — not whether
structure exists.

Return valid JSON only — no markdown fences: {"verdict": "human" | "ai", "human_score": N, "tells": ["..."]}

human_score (0-10): 0 = obviously AI-written, 10 = indistinguishable from a real practitioner's
unpolished writing. Be strict on PROSE QUALITY specifically — generic, evenly-paced, hedge-
everything paragraphs should score below 6 even if the article is well-organized.
"tells" lists the specific things that gave it away (empty list if genuinely human-sounding).\
"""
...
def _strip_em_dash(text: str) -> str:
    """Belt-and-suspenders: the model is told never to use an em dash, but rewrite
    any that slip through instead of shipping them."""
...
def write_post(story: dict, original: dict | None, client: anthropic.Anthropic) -> str | None:
    ...
    try:
        comment = json.loads(raw).get("comment")
    except json.JSONDecodeError:
        log.error("Writing LLM returned invalid JSON: %s", raw)
        return None
    return _strip_em_dash(comment) if comment else None
...
def check_human_voice_longform(text: str, client: anthropic.Anthropic) -> dict:
    """Score whether long-form blog prose reads as human- or AI-written. Separate prompt and
    threshold from check_human_voice() — a multi-paragraph article legitimately has more
    template-shaped structural elements (a callout, a checklist) than a 6-line LinkedIn post,
    which would false-positive against a prompt calibrated for short-post AI tells."""
    msg = client.messages.create(
        model="claude-haiku-4-5-20251001",
        max_tokens=200,
        temperature=0,
        system=_HUMANNESS_SYSTEM_LONGFORM,
        messages=[{"role": "user", "content": f"ARTICLE PROSE:\n{text}"}],
    )
    raw = _strip_json_fences(msg.content[0].text)
    try:
        return json.loads(raw)
    except json.JSONDecodeError:
        log.warning("Longform humanness checker returned invalid JSON: %s — assuming human", raw)
        return {"verdict": "human", "human_score": 10, "tells": []}
