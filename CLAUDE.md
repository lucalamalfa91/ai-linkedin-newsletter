...
7. **`utils/diagram_renderer.py`** — renders each `diagram`-type block to a PNG under `site/posts/images/`: real HTML/CSS + inline SVG icons, screenshotted with headless Chromium via Playwright (`render_flow_diagram()` for a linear flow, `render_compare_table()` for the `comparison` archetype's optional trade-off table). A random theme is picked per render (`random_theme()`) — deliberately not tied to the technique, or every article on the same topic would render the same color. Best-effort — a failed render just drops that diagram block. Bundled Liberation Sans font under `assets/fonts/` keeps typography consistent across machines. Every article also gets one cover/hero image (`render_hero_image()`, `<slug>-hero.png`), used as the homepage thumbnail, `og:image`/Twitter Card image, and RSS enclosure: `agents/site_writer_agent.generate_image_prompt()` asks Claude for a short SUBJECT description specific to that article (not just its technique category), which is combined with a fixed `IMAGE_STYLE_SUFFIX` (flat minimalist vector illustration, navy/indigo-blue palette matching the site's own design) and sent to a free, keyless AI image API (`AI_IMAGE_API_URL` in `config.py`, no account or cost). If that fails for any reason (no SLA on a free service), falls back to a branded flat-color card with a technique-specific icon (`TECHNIQUE_ICONS`) — never blocks publishing either way.
...
- **Hero image prompt** (`site_writer_agent.py`, `generate_image_prompt`): `claude-haiku-4-5-20251001`, max_tokens=60, temperature=0.9 — writes only the article-specific SUBJECT of the cover image (style is fixed separately in `utils/diagram_renderer.IMAGE_STYLE_SUFFIX`, not left to the model)
...
- Hero images (`utils/diagram_renderer.render_hero_image`) call a free, keyless external image-generation API (`config.AI_IMAGE_API_URL`, currently Pollinations.ai) via `requests` — no account, no cost, no SLA; the flat-color card render is the fallback if it's ever unreachable
